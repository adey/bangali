...
//	sendEvent(name: "temp_f", value: obs.current.temp_f, unit: "F")
	...
	if(isStateChange(device, "temperature", t0.toString())) {	
...
//	}
	...
	sendEvent(name: "temperatureHighDayPlus1", value: (!wantMetric() ? obs.forecast.forecastday[0].day.maxtemp_f :
							obs.forecast.forecastday[0].day.maxtemp_c), unit: "${(!wantMetric() ? 'F' : 'C')}", /* isStateChange: true, */ displayed: true)
	sendEvent(name: "temperatureLowDayPlus1", value: (!wantMetric() ? obs.forecast.forecastday[0].day.mintemp_f :
							obs.forecast.forecastday[0].day.mintemp_c), unit: "${(!wantMetric() ? 'F' : 'C')}", /* isStateChange: true, */ displayed: true)
	return
}
...
private getXUdata() {
	def obs = [:]
	def params = [ uri: "https://api.apixu.com/v1/forecast.json?key=${apixuKey}&q=${state?.zipCode}&days=3" ]
	try {
		httpGet(params)		{ resp ->
